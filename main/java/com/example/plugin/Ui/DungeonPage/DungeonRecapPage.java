...
public class DungeonRecapPage extends InteractiveCustomUIPage<DungeonRecapPage.Data> {
    ...
    public void build(@Nonnull Ref<EntityStore> ref, @Nonnull UICommandBuilder cmd,
        ...
        for (short i = 0; i < 36; i++) {
            ...
            ItemStack item = null;
            try { item = inventory.getItemStack(i); } catch (Exception ignored) {}
        ...
        for (short i = 0; i < 9; i++) {
            ...
            ItemStack item = null;
            try { item = hotbar.getItemStack(i); } catch (Exception ignored) {}
        ...
        if (currentSelection != null) {
            try {
                ...
                if (selected != null && !ItemStack.isEmpty(selected)) {
                    cmd.appendInline("#StashTargetSlot", "ItemSlot { ItemId: \"" + selected.getItemId() + "\"; Anchor: (Full: 0); ShowQuantity: true; }");
                }
            } catch (Exception ignored) {}
        }

        // Bind the new side-by-side buttons
        evt.addEventBinding(CustomUIEventBindingType.Activating, "#QuickDepositBtn", EventData.of("ButtonClicked", "deposit_selected"), false);
        evt.addEventBinding(CustomUIEventBindingType.Activating, "#QuickDepositAllBtn", EventData.of("ButtonClicked", "transfer_all"), false);
        evt.addEventBinding(CustomUIEventBindingType.Activating, "#CloseBtn", EventData.of("ButtonClicked", "close_recap"), false);
    }
    
    private void buildSlot(UICommandBuilder cmd, UIEventBuilder evt, String groupId, String btnId, ItemStack item, String selectionId, String currentSelection) {
        if (item != null && !ItemStack.isEmpty(item)) {
            cmd.appendInline(groupId, "ItemSlot { ItemId: \"" + item.getItemId() + "\"; Anchor: (Full: 0); ShowQuantity: true; }");
            ...
            if (itemValue > 0) {
                ...
                tooltip += "Will be sold automatically for: " + totalStackValue + " Gold";
                
                cmd.appendInline(groupId, "TextButton #" + btnId + " { Anchor: (Full: 0); Text: \"\"; Background: #ff0000(0.15); TooltipText: \"" + tooltip + "\"; }");
            } else {
                cmd.appendInline(groupId, "TextButton #" + btnId + " { Anchor: (Full: 0); Text: \"\"; Background: #000000(0.0); TooltipText: \"" + tooltip + "\"; Style: (Hovered: (Background: #254a7588)); }");
                evt.addEventBinding(CustomUIEventBindingType.Activating, "#" + btnId, EventData.of("ButtonClicked", "select_" + selectionId), false);
            }
            
        } else {
            cmd.appendInline(groupId, "TextButton #" + btnId + " { Anchor: (Full: 0); Text: \"\"; Background: #000000(0.0); }");
        }
    ...
    public void handleDataEvent(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, Data data) {
        ...
        if (action.equals("deposit_selected")) {
            ...
            ItemStack item = null;
            try { item = targetSource.getItemStack(index); } catch (Exception ignored) {}
    ...
    private void transferToStash(ItemContainer source, short sourceIndex, ItemContainer stash) {
        try {
            ...
            if (SellConfig.getItemSellValue(item) > 0) return;
            short freeSlot = findFreeStashSlot(stash);
            if (freeSlot != -1) {
    ...
    private void refresh(Player player, Ref<EntityStore> ref, Store<EntityStore> store) {
        player.getPageManager().openCustomPage(ref, store, new DungeonRecapPage(this.playerRef, this.statsText, this.dungeonSlot));
    }
}